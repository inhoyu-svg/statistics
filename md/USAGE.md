...
### 📊 변량 테이블 (Variable Table)

격자선 없이 둥근 테두리만 표시하는 간단한 테이블입니다.

**특징:**
- 격자선 대신 둥근 테두리로 데이터 영역 감싸기
- 헤더, 합계 행 없이 데이터만 표시
- `showGrid: false` 옵션으로 활성화

**적합한 데이터:** 변량(자료값) 나열, 간단한 데이터 목록

**입력 형식:**
```
헤더: null, null, null, null, null
A반: 1, 2, 3, 4, 5
```
