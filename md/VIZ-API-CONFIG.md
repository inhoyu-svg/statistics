...
> 2. **막대 꼭대기 기준**: 각 막대의 도수/상대도수 높이에서 **밑으로 1~2칸만** 찢기 (막대 전체 X)
> 3. **양옆 마진 제한**: 마진 1칸은 **해당 막대 높이 범위 내에서만** 찢기
>
> **📐 계단식 찢김 예시** (idx 2, 3, 4를 가리고, 각각 도수 11, 11, 8인 경우):
> ```
> idx:     0    1    2    3    4    5    6
> y=11   □□  □□  ■■  ■■  □□  □□  □□   ← idx 2, 3의 꼭대기
> y=10   □□  ■■  ■■  ■■  □□  □□  □□   ← idx 1은 마진 (높이 범위 내만)
> y=9    □□  ■■  ■■  ■■  ■■  □□  □□   ← idx 2, 3, 4 공통 범위
> y=8    □□  □□  ■■  ■■  ■■  □□  □□   ← idx 4의 꼭대기
> y=7    □□  □□  ■■  ■■  ■■  ■■  □□   ← idx 5는 마진 (높이 범위 내만)
> y=6    □□  □□  □□  ■■  ■■  □□  □□   ← 더 내려가면 다른 막대 데이터 노출 위험
> ```
> - ⛔ idx 1 막대의 실제 데이터 y=4는 건드리면 안됨 → 마진은 y=10~9만
> - ⛔ idx 5 막대의 실제 데이터 y=3은 건드리면 안됨 → 마진은 y=8~7만
>
> **💡 마진 규칙**: 양옆 1칸 마진은 **인접 막대의 실제 데이터보다 높은 y좌표에서만** 적용
> - 예: idx 1의 도수가 4이면 → idx 1 마진은 y=5 이상에서만 찢기
...
**시나리오**: idx 2, 3, 4를 가려야 하고, 도수가 각각 11, 11, 8인 경우
- idx 1 (마진 왼쪽)의 도수: 4 → 마진은 y=5 이상만
- idx 5 (마진 오른쪽)의 도수: 3 → 마진은 y=4 이상만

```json
"cells": [
  "0-13:8-13",   // 상단 전체 (y=13이 최대)
  "1-12:7-12",   // 살짝 좁아짐
  "1-11:7-11",   // idx 2, 3 꼭대기 (도수 11)
  "2-10:6-10",   // idx 2, 3 범위
  "2-9:5-9",     // idx 2, 3, 4 공통 범위
  "2-8:5-8",     // idx 4 꼭대기 (도수 8)
  "2-7:4-7",     // idx 4 범위
  "3-6:4-6",     // idx 4 아래
  "3-5"          // 끝점 (idx 1 도수 4보다 위에서 끝남)
]
```
