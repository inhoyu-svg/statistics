...
> 2. **막대 꼭대기 포함**: 각 막대의 도수 y좌표를 **포함**하고, 그 아래로 1~2칸까지 찢기
>    - ⚠️ "꼭대기 위"가 아니라 **"꼭대기를 덮고 아래로"**!
>    - 예: 도수 11인 막대 → y=11, 10, 9 찢기 (y=12만 찢으면 막대가 보임!)
> 3. **양옆 마진 제한**: 마진 1칸은 **인접 막대 데이터보다 높은 y좌표에서만** 찢기
>
> **📐 계단식 찢김 예시** (idx 2, 3을 가리고, 각각 도수 12, 10인 경우):
> ```
> idx:     0    1    2    3    4    5    6
> y=13   □□  ■■  ■■  ■■  ■■  □□  □□   ← 상단 (마진 포함)
> y=12   □□  ■■  ■■  ■■  ■■  □□  □□   ← idx 2 꼭대기 (도수 12) ★포함★
> y=11   □□  ■■  ■■  ■■  ■■  □□  □□   ← idx 2 아래
> y=10   □□  □□  ■■  ■■  ■■  □□  □□   ← idx 3 꼭대기 (도수 10) ★포함★
> y=9    □□  □□  ■■  ■■  □□  □□  □□   ← idx 3 아래 (끝점)
> y=8    □□  □□  □□  □□  ██  □□  □□   ← idx 4 실제 데이터 (안 건드림!)
> y=4    □□  ██  □□  □□  □□  □□  □□   ← idx 1 실제 데이터 (안 건드림!)
> ```
> - ✅ idx 2 도수=12 → y=12를 **반드시 포함** (y=13만 찢으면 막대 보임!)
> - ✅ idx 3 도수=10 → y=10을 **반드시 포함**
> - ⛔ idx 1 도수=4 → 마진은 y=5 이상에서만
> - ⛔ idx 4 도수=8 → 마진은 y=9 이상에서만
>
> **💡 핵심**: 막대의 꼭대기 y좌표 = 해당 막대의 도수값. **이 y좌표를 반드시 찢어야 막대가 가려짐!**
...
```
- 타겟 막대: idx 2, 3
- 왼쪽 마진 1칸: idx 1 추가 → x 범위: 1~3
- 오른쪽 마진 1칸: idx 4 추가 → x 범위: 1~4
- 상단은 더 넓게: x 범위: 0~8 (차트 전체 폭 커버)
```

**3단계: y좌표 결정 (위에서 아래로)**
```
1. 상단 시작: y = gridDivisions - 1 (예: 13)
2. 타겟 막대 꼭대기: y = 각 막대 도수 (예: 11)
3. 끝점: 인접 막대 도수 + 1 (예: idx 4 도수 8 → y=9까지만, idx 1 도수 4 → y=5까지만)
```
...
각 y좌표마다:
- 해당 y에서 찢을 x 범위 결정
- 마진 idx는 해당 idx의 도수보다 높은 y에서만 포함
- "x시작-y:x끝-y" 형식으로 작성
```
...
**시나리오**:
- 막대 7개 (idx 0~6)
- idx 2, 3을 가려야 함 (도수 각각 11, 11)
- idx 1 도수: 4, idx 4 도수: 8
...
**cells 배열 해설:**
```
"0-13:8-13"   → y=13: 상단 전체 (x: 0~8)
"1-12:7-12"   → y=12: 살짝 좁아짐 (x: 1~7)
"1-11:7-11"   → y=11: idx 2, 3 꼭대기 ★도수 11 포함★ (x: 1~7, 마진 포함)
"2-10:6-10"   → y=10: 좁아짐 (x: 2~6)
"2-9:5-9"     → y=9: idx 4 도수 8 + 1 (x: 2~5)
"2-8:4-8"     → y=8: idx 4 꼭대기 ★도수 8 포함★
"2-7:4-7"     → y=7: 계속 좁아짐
"3-6:4-6"     → y=6: 더 좁아짐
"3-5"         → y=5: 끝점 (idx 1 도수 4 + 1)
```

**시각화:**
```
y(그리드)  idx: 0    1    2    3    4    5    6    7    8
  13 |         ■■  ■■  ■■  ■■  ■■  ■■  ■■  ■■  ■■   ← 상단 전체
  12 |         □□  ■■  ■■  ■■  ■■  ■■  ■■  ■■  □□   ← 살짝 좁아짐
  11 |         □□  ■■  ■■  ■■  ■■  ■■  ■■  ■■  □□   ← idx 2, 3 꼭대기 (도수 11) ★포함★
  10 |         □□  □□  ■■  ■■  ■■  ■■  ■■  □□  □□   ← 좁아짐
   9 |         □□  □□  ■■  ■■  ■■  ■■  □□  □□  □□   ← idx 4 도수 8 + 1
   8 |         □□  □□  ■■  ■■  ■■  □□  □□  □□  □□   ← idx 4 꼭대기 (도수 8) ★포함★
   7 |         □□  □□  ■■  ■■  ■■  □□  □□  □□  □□
