...
#### englishFont (영어 폰트 - 차트/산점도)

차트(히스토그램)와 산점도에서 축 제목과 콜아웃 텍스트에 Source Han Sans KR 폰트를 사용합니다.
...
| 모드 | X축 최댓값 라벨 | X축 제목 위치 |
|:-----|:--------------|:-------------|
| `false` (기본) | 축 제목으로 대체 | 최댓값 위치 |
| `true` | 그대로 표시 | 한 칸 아래 + 오른쪽 정렬 |

**폰트 규칙**:
- 짧은 변수명 (1-2글자: `x`, `y`, `AB`): KaTeX 이탤릭/일반 폰트 유지
- 긴 영어 단어 (3글자 이상: `score`, `test`): Source Han Sans KR 사용
