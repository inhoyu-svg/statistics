...
| **탈리마크 있으면 `헤더명*2` 필수** | `도수*2` → 탈리+숫자 2열 병합 헤더 |
...
**data 형식 설명**:
- `헤더: null, null, null, null, null` → 5열 모두 빈칸
- `200: 190, 185, 195, 195` → 첫 열 200, 나머지 열에 변량값들
...
탈리마크와 숫자를 함께 표시할 때는 **헤더 병합(`*2`)을 사용**해야 합니다.
...
**헤더 병합 문법:**
- `도수*2`: "도수" 헤더가 2열을 병합 (탈리마크 열 + 숫자 열)
- 데이터는 2개씩 입력: `/////, 5` (탈리마크, 도수)
...
> **중요**: 탈리마크가 있는 테이블은 반드시 `헤더명*2` 형식을 사용하세요.

**합계 행 처리:**
탈리마크 열은 숫자가 아니므로 합계가 `-`로 표시됩니다. `cellVariables`로 직접 지정하세요.
...
```json
{
  ...
  "data": "헤더: 혈액형, 도수*2\nA: /////, 5\nB: ///, 3\nAB: //, 2\nO: ///////, 7",
  "cellVariables": [
    { "rowIndex": 5, "colIndex": 1, "value": null }
  ]
}
```

- `rowIndex: 5` = 합계 행 (헤더 제외, 데이터 4행 + 합계 1행)
- `colIndex: 1` = 탈리마크 열
- `value: null` = 빈칸 처리
