...
| `canvasHeight` | `number` | X | `600` | 캔버스 높이 (px) |
...
데이터가 0에서 멀리 떨어져 있으면 자동으로 축이 압축됩니다.

**압축 조건**: 첫 데이터 값이 전체 범위의 30% 이상 떨어져 있을 때

**예시**: X 범위 0~100, 첫 데이터 X값이 40 이상이면 압축 적용

압축 구간에는 이중물결표(≈) 기호가 표시됩니다.
