...
| 조건 | 색상 | 프리셋 |
|:-----|:-----|:-------|
| 단일 그룹 하이라이트 | 녹색 | `default` |
| 복수 그룹 - 첫 번째 | 파랑 | `primary` |
| 복수 그룹 - 두 번째 | 빨강 | `secondary` |
| 복수 그룹 - 세 번째 | 주황 | `tertiary` |
...
|:-----|:----------|:----------|
| 히스토그램만 | `"green"` | - |
| 도수다각형만 (단일) | - | `"default"` (녹색) |
| 도수다각형만 (복수) | - | `"primary"` → `"secondary"` → `"tertiary"` (녹색 제외) |
| 히스토그램 + 도수다각형 | `"default"` (파랑) | `"default"` → `"secondary"` → `"tertiary"` |

> **중요**: 히스토그램과 도수다각형이 함께 사용될 때는 **동일한 색상 계열을 사용하지 않습니다.**
> - 히스토그램: 파랑 계열 (`"default"`)
> - 도수다각형: 녹색/빨강/주황 계열 (`"primary"` 파랑 제외)
...
┌─────────────────────────────────────────────────────────────┐
│  히스토그램 단독        → 녹색 (green)                      │
│  도수다각형 단독 (1개)  → 녹색 (default)                    │
│  도수다각형 단독 (복수) → 파랑 → 빨강 → 주황 (녹색 X)       │
│  히스토그램 + 다각형    → 히스토그램: 파랑 / 다각형: 녹색~  │
└─────────────────────────────────────────────────────────────┘
```
