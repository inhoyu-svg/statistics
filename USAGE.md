...
차트가 생성되면 애니메이션으로 표시됩니다. 애니메이션을 제어할 수 있습니다.
...
**속도 조절:**
- 슬라이더로 0.5x ~ 3.0x 속도 조절 가능
- 기본 속도: 1.0x

**진행도 슬라이더:**
- 드래그하여 원하는 시점으로 이동
- 애니메이션 진행 상태를 실시간으로 표시

**활용:**
- 프레젠테이션 시 데이터를 단계별로 설명
- 특정 구간에 집중하여 분석
- 빠른 속도로 전체 흐름 파악

### 레이어 관리 패널 🎨

차트의 각 요소(막대, 점, 선)를 개별적으로 제어할 수 있습니다.

**레이어 종류:**
- **막대 (히스토그램)**: 각 계급의 도수를 나타내는 막대
- **점**: 상대도수 다각형의 각 지점
- **선**: 점을 연결한 다각형 선

**레이어 제어:**
- **눈 아이콘 (👁)**: 클릭하여 표시/숨김 전환
- **드래그 핸들 (⋮⋮)**: 레이어를 드래그하여 렌더링 순서 변경
- **레이어 이름**: 각 레이어의 타입 표시

**활용:**
- 막대만 보기 (히스토그램 집중)
- 선과 점만 보기 (다각형 집중)
- 레이어 순서 변경하여 원하는 요소를 위로 표시

### 차트 데이터 타입 선택 📊

차트에 표시할 데이터 종류를 선택할 수 있습니다.

**데이터 타입:**
1. **상대도수 (%)**: 기본값, 전체 대비 비율 표시
   - Y축: 0 ~ 100%
   - 막대 높이: 각 계급의 비율
...
도수분포표의 컬럼을 자유롭게 설정할 수 있습니다.

**고급 설정 열기:**
1. 왼쪽 입력 섹션에서 "고급 설정" 클릭
2. "표 컬럼 설정" 섹션 확인

**컬럼별 제어:**
- **체크박스**: 컬럼 표시/숨김
- **드래그 핸들 (⋮⋮)**: 컬럼 순서 변경
- **정렬 버튼 (L/C/R)**: 좌측/중앙/우측 정렬
- **라벨 입력칸**: 컬럼 이름 변경
...
**설정 방법:**
- 왼쪽 입력 섹션에서 "첫 계급에 상첨자 표시 (이상/미만)" 체크박스
- 기본값: 체크됨
...
특정 구간을 직접 설정하여 계급을 생성할 수 있습니다.

**설정 방법:**
1. 도수분포표 생성 후 차트 아래 "📝 계급 범위 커스터마이징" 섹션 확인
2. 세 가지 값 입력:
   - **첫 칸**: 0 ~ (입력값)
   - **두 번째**: (첫 칸 끝) ~ (입력값) → 간격 결정
   - **마지막**: (입력값) ~ (자동 계산)
...
- 특정 구간을 강조하고 싶을 때
- 불규칙한 계급 간격이 필요할 때
- 시험 점수 구간 등 사용자 정의 범위

### X축/Y축 라벨 변경

차트의 축 라벨을 자유롭게 변경할 수 있습니다.

**설정 방법:**
1. 왼쪽 입력 섹션에서 "고급 설정" 클릭
2. X축 라벨 / Y축 라벨 입력란에 원하는 텍스트 입력

**예시:**
```
기본값:
X축: "계급"
Y축: "상대도수" 또는 "도수" (데이터 타입에 따라)
...
**주의사항:**
- X축 라벨은 테이블의 "계급" 컬럼 이름과 연동됨
- Y축 라벨은 차트 데이터 타입에 따라 자동 조정됨
