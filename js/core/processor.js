...
class DataProcessor {
  ...
  /**
   * 여러 데이터셋을 위한 공통 계급 및 데이터셋별 도수 계산
   * @param {Array} datasets - 데이터셋 배열 (DataStore.getAllDatasets() 반환값)
   * @param {number} classCount - 생성할 계급 개수
   * @param {number|null} customWidth - 커스텀 계급 간격 (선택)
   * @param {Object|null} customRange - 커스텀 범위 설정
   * @returns {Object} { classes, datasets, classWidth }
   * @description
   * - 모든 데이터셋의 min/max를 고려하여 공통 계급 범위 생성
   * - 각 데이터셋별로 도수/상대도수 계산
   * - 반환: { classes: 공통 계급 배열, datasets: 데이터셋별 도수 정보, classWidth }
   */
  static createClassesForMultipleDatasets(datasets, classCount, customWidth = null, customRange = null) {
    // 데이터가 있는 데이터셋만 필터링
    const validDatasets = datasets.filter(d => d.data && d.data.length > 0);

    if (validDatasets.length === 0) {
      throw new Error('유효한 데이터셋이 없습니다.');
    }

    // 모든 데이터셋의 통계 계산
    const allStats = validDatasets.map(dataset => ({
      ...dataset,
      stats: this.calculateBasicStats(dataset.data)
    }));

    // 공통 min/max 계산
    const globalMin = Math.min(...allStats.map(d => d.stats.min));
    const globalMax = Math.max(...allStats.map(d => d.stats.max));
    ...
    // 공통 계급 생성
    const { classes, classWidth } = this.createClasses(globalStats, classCount, customWidth, customRange);

    // 각 데이터셋별로 도수 계산
    const datasetResults = allStats.map(dataset => {
      // 계급 복사 (각 데이터셋마다 독립적인 도수 계산)
      const datasetClasses = classes.map(c => ({
        min: c.min,
        max: c.max,
        frequency: 0,
        data: [],
      ...
      // 도수 계산
      this.calculateFrequencies(dataset.data, datasetClasses);

      // 상대도수 및 누적도수 계산
      this.calculateRelativeAndCumulative(datasetClasses, dataset.data.length);
    ...
    return {
      classes,           // 공통 계급 정보 (구조만, 도수는 없음)
      datasets: datasetResults,  // 데이터셋별 도수 정보
      classWidth
    };
  }
